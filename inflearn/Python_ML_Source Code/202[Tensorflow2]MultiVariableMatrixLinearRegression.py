...
marathon_2015_2017 = pd.read_csv(os.path.join(
    BASE_DIR, 'data\marathon_2015_2017.csv'))


# Merge 2015, 2016 and 2017 files into marathon_2015_2017 file index by Official Time
record = pd.DataFrame(marathon_2015_2017, columns=[
                      'M/F',  'Age',  'Pace',  '10K', '20K',  '30K',  'Official Time']).sort_values(by=['Official Time'])
...
def histogram():
    ...
    grad_ax.annotate("%10s %7i" %
                     ('Count : ', stat[0]), (75, 1200), fontsize=10)
    grad_ax.annotate("%10s %7.3f" %
                     ('Mean :  ', stat[1]), (75, 1150), fontsize=10)
    grad_ax.annotate("%10s %7.3f" %
                     ('25% :   ', stat[3]), (75, 1100), fontsize=10)
    grad_ax.annotate("%10s %7.3f" %
                     ('75% :   ', stat[5]), (75, 1050), fontsize=10)
...
def learing():
    ...
    log_ScrolledText.insert(END, "%10s %10s %10s %50s" % (
        'Gender', 'Age', 'Pace', 'Record Prediction(Second) at 42.195km')+'\n\n')
    log_ScrolledText.insert(END, "%10s %10s %10s %50s" % (
        gender_list[t_g], str(t_a), str(t_p), ml_time)+'\n')
...
t_aSpbox = Spinbox(main, textvariable=t_aVal, from_=18,
                   to=84, increment=1, justify=RIGHT)
...
t_pSpbox = Spinbox(main, textvariable=t_pVal, from_=0,
                   to=1500, increment=1, justify=RIGHT)
...
t_tSpbox = Spinbox(main, textvariable=t_tVal, from_=0,
                   to=100000, increment=1000, justify=RIGHT)
# t_tSpbox.config(state='readonly')
...
t_rSpbox = Spinbox(main, textvariable=t_rVal, from_=0,
                   to=1, increment=1e-6, justify=RIGHT)
# t_rSpbox.config(state='readonly')
t_rSpbox.grid(row=2, column=3)
t_rLabel = Label(main, text='Learning rate : ')
t_rLabel.grid(row=2, column=2)

Button(main, text="Histogram", height=2, command=lambda: histogram()).grid(
    row=2, column=4, columnspan=1, sticky=(W, E))
Button(main, text="Prediction", height=2, command=lambda: learing()).grid(
    row=2, column=5, columnspan=1, sticky=(W, E))
...
log_ScrolledText.tag_config(
    'RESULT', foreground='blue', font=("Helvetica", 12))
log_ScrolledText.tag_config(
    'HEADER', foreground='gray', font=("Helvetica", 14), underline=1)
